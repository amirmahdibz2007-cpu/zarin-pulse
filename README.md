...
معماری و مرزهای پکیج: [`docs/architecture.md`](docs/architecture.md).
...
بدون `OPENROUTER_API_KEY` صفحهٔ `/ai` همچنان کار می‌کند (بریف و چت از روی artifact قفل‌شده). با کلید، مدل زندهٔ OpenRouter (رایگان) اولویت دارد.
اختیاری: `ARTIFACTS_ROOT` برای مسیر صریح درخت artifact (مثلاً در Docker).
